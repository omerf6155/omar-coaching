...
function renderWorkoutView(dayKey) {
    ...
    plan.exercises.forEach(ex => {
        const savedSeat = (appData.seatSettings && (appData.seatSettings[ex.id] || (ex.name && appData.seatSettings[ex.name]))) || ex.defaultSeat || '';
        const lastLog = (appData.workoutLogs && (appData.workoutLogs[ex.id] || (ex.name && appData.workoutLogs[ex.name]))) || [];
    ...
    dayKeys.forEach((dayKey, idx) => {
        ...
        if (plan) {
            ...
            if (plan.exercises && plan.exercises.length > 0) {
                ...
                plan.exercises.forEach(ex => {
                    const savedSeat = (appData.seatSettings && (appData.seatSettings[ex.id] || (ex.name && appData.seatSettings[ex.name]))) || ex.defaultSeat || '--';
                    const logs = (appData.workoutLogs && (appData.workoutLogs[ex.id] || (ex.name && appData.workoutLogs[ex.name]))) || [];
