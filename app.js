...
function renderCoachRoster() {
    ...
    athletes.forEach(ath => {
        ...
        const consumedCal = (typeof n.calories === 'number' ? n.calories : (n.consumedCal || 0));
...
function renderCoachDetail(username) {
    ...
    if (calEl) calEl.innerText = `${(typeof n.calories === 'number' ? n.calories : (n.consumedCal || 0))} / ${t.calories || 2770}`;
    ...
    if (currentCoachDetailSubtab === "nutrition") {
        ...
        const dispCal = typeof n.calories === 'number' ? n.calories : (n.consumedCal || 0);
        const dispP = typeof n.protein === 'number' ? n.protein : (n.consumedP || 0);
        const dispC = typeof n.carbs === 'number' ? n.carbs : (n.consumedC || 0);
        const dispF = typeof n.fat === 'number' ? n.fat : (n.consumedF || 0);
        ...
        if (plan) {
            ...
            if (plan.exercises && plan.exercises.length > 0) {
                ...
                plan.exercises.forEach(ex => {
                    const cKey = getCanonicalExerciseKey(ex.name, ex.id);
                    const savedSeat = (appData.seatSettings && (appData.seatSettings[ex.id] || (ex.name && appData.seatSettings[ex.name]) || appData.seatSettings[cKey])) || ex.defaultSeat || '--';
                    const logs = (appData.workoutLogs && (appData.workoutLogs[ex.id] || (ex.name && appData.workoutLogs[ex.name]) || appData.workoutLogs[cKey])) || [];
